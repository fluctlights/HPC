...
DIRECTORY = "kmeans/sequential/macqueenV2"
...
class MacQueen:
    ...
    def _calculate_distance(self, X, centroids, distance_metric='euclidean'):
        """Calculate distance between each point in X and each centroid."""
        if distance_metric == 'euclidean':
            return np.sqrt(np.sum((X[:, np.newaxis] - centroids) ** 2, axis=2))
        elif distance_metric == 'manhattan':
            return np.sum(np.abs(X[:, np.newaxis] - centroids), axis=2)
        elif distance_metric == 'chebyshev':
            return np.max(np.abs(X[:, np.newaxis] - centroids), axis=2)
        else:
    ...
    def _assign_clusters(self, X, centroids, distance_metric='euclidean'):
        ...
        distances = self._calculate_distance(X, centroids, distance_metric)
