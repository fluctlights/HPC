...
const string DIRECTORY = "kmeans/OpenMP/V1/";
...
double calculate_distance(const float* point1, const float* point2, int n_features, int distance_metric) {
    ...
    if (distance_metric == EUCLIDEAN) {
        ...
        for (int i = 0; i < n_features; ++i) {
            distance += std::fabs(point1[i] - point2[i]);
        }
    } else if (distance_metric == CHEBYSHEV) {
        double max_diff = 0.0;
        for (int i = 0; i < n_features; ++i) {
            double diff = std::fabs(point1[i] - point2[i]);
            if (diff > max_diff) {
                max_diff = diff;
            }
        }
...
void assign_clusters(const Mat& data, int k, const Mat& centroids, Mat& labels, int distance_metric) {
    for (int i = 0; i < data.rows; ++i) {
        ...
        labels.at<int>(i, 0) = best_cluster;
    }
}

void update_centroids(const Mat& data, const Mat& labels, int k, Mat& centroids) {
